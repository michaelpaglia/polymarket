...
use hft_core::{ArbitrageConfig, ArbitrageDetector, HftConfig, TradingState};
...
async fn main() -> Result<()> {
    ...
    let api_key = std::env::var("POLYMARKET_API_KEY")
        .expect("POLYMARKET_API_KEY must be set");
    let api_secret = std::env::var("POLYMARKET_API_SECRET")
        .expect("POLYMARKET_API_SECRET must be set");
    let api_passphrase = std::env::var("POLYMARKET_API_PASSPHRASE")
        .expect("POLYMARKET_API_PASSPHRASE must be set");
    ...
    info!(bind = %api_bind, "API server started");
